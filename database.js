...
module.exports = {
  ...
  */
  ...
  async addSpotifyId(query, newid) {
  ...
  async addPlaylist(trackarray, listname) {
  ...
  async updateTrack(query, update) {
  ...
  async switchAlternate(query, alternate) {
  // returns the first track object that matches the query
    try {
      const tracks = db.collection(trackcol);
  ...
  async newUser(discord) { // usage: await newUser({ id:'119678070222225408', username:'Ethazeriel', nickname:'Eth', discriminator:'4962', guild:'888246961097048065', locale:'en-US'});
  ...
  async getUser(discordid) { // usage: const result = await getUser('119678070222225408');
  ...
  async updateUser(discordid, field, data, guild) { // usage: const result = await database.updateUser(userid, 'username', member.user.username);
  ...
  async saveStash(discordid, playhead, queue) { // usage: const result = await saveStash('119678070222225408', player.getPlayhead(), player.getQueue());
  ...
  async getStash(discordid) { // usage: const result = await getStash('119678070222225408');
  ...
  async genericUpdate(query, changes, collection) {
  // generic update function; basically just a wrapper for updateOne
    try {
  ...
  async genericGet(query, collection) { // arc v1
