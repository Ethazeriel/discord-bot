...
import { log, sanitize, youtubePattern, sanitizePlaylists, db, Player } from '../../internal.js';
  ...
  if ((interaction.member?.roles as GuildMemberRoleManager)?.cache?.some(role => role.name === roles.admin)) {
    ...
    switch (interaction.options.getSubcommand()) {
      ...
      case 'removeplaylist': {
        ...
        const result = await db.removePlaylist(listname);
      ...
      case 'removetrack': {
        ...
        if (youtubePattern.test(track)) {
          ...
          const dbtrack = await db.getTrack({ 'youtube.0.id':match![2] });
          if (dbtrack) { Player.removeFromAll(dbtrack.goose.id); }
          const result = await db.removeTrack(match![2]);
